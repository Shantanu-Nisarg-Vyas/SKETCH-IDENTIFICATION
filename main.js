...
function setup() {
    ...
    canvas.position(1000, 575);
    background("white");
    //canvas.mouseReleased(classifyCanvas);
    synth = window.speechSynthesis;
    classifier = ml5.imageClassifier("DoodleNet",classifyCanvas);
}
...
function gotResult(error, results) {
    ...
    utterThis = new SpeechSynthesisUtterance("The prediction is" + results[0].label);
    synth.speak(utterThis);
    classifier.classify(canvas,gotResult);    
    
    if (randomobj == results[0].label) {
        score = score + 1;
